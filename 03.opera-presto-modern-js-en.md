...
[Last time]({{ '/revival/' | relative_url }}#xii-the-beautiful-opera-of-the-future) I finished by saying that we need **ES2015**. Without upgrading Carakan (which knows ECMAScript 5.1 perfectly well, but nothing about newer specs), we won't just fail to use the modern web—we won't even be able to run many tests.
