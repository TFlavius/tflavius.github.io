...
The `search_engine` module makes this easy: its author left [a full product presentation]({{ '/revival/' | relative_url }}#xi-more-gems-and-more-madness) right there in the source tree. The module is not a relational DBMS and has no SQL, no schemas, no query planner, and no general-purpose secondary index system. Its strength is a handful of structures chosen in advance for the specific operations Opera performs:
...
Even so, I expect to [add IndexedDB to Opera]({{ '/indexeddb-hwaccel/' | relative_url }}#xxxvi-indexeddb-is-in-place) and the tools for working with it to Dragonfly. That work has already begun, and the finish line is in sight.

After that, with no dates promised, comes **ECMAScript 2026**, including Unicode 17, the current RegExp features, [the new data types]({{ '/es2026-p1/' | relative_url }}#xlix-big-whole-and-uncut), and [all of Intl]({{ '/es2026-p1/' | relative_url }}#l-lost-in-translation). It sounds wildly ambitious, but without it everything already done somehow loses its point.
