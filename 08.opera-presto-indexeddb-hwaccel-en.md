...
The threading itself is entirely conventional: `CreateThread()` on Windows and `PosixThread::CreateThread()` on Linux, with a new `OpWorkerThread` abstraction settling in next to the existing `OpPseudoThread` ([which we will have to come back to]({{ '/pseudothread/' | relative_url }}#xl-single-threaded-multithreading)). That abstraction will be far easier to swap for a cross-process one, if we ever get that far.
...
The good news: the browser already passes 72,254 of the 81,178 tests in the ES2026 edition. The bad news: a single `test262` run on a debug build of Opera takes [**~30 hours**]({{ '/es2026-p1/' | relative_url }}#xlviiiii-thirty-dreary-hours). Release builds are faster, but catching bugs means running the debug one, and that is honestly wearing me down.
